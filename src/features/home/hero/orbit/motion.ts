 ...
 * a second copy ("B") that is invisible except during its own wrap step. At
 * the start of that step the two swap: B grows and fades in at the entry slot
 * over the step's first eighth (WRAP_SWAP), then copy A shrinks and fades out
 * at slot 8 over the second, and B rides in to slot 1 at full size and
 * strength.
 * At the loop point the two copies hand over at slot 1 with identical pixels,
...
/** The satellites' drift period. They read `--brain-period` first, so it can
 *  be retimed on the stage. (The orange raster brain that shared it is gone;
 *  the glass brain's turn is PERIOD_S, see HeroBrain.) */
...
/** Samples across the wrap step, where scale and opacity ease. Sixteen, so
 *  the swap (WRAP_SWAP) can be one sample long. */
const WRAP_SAMPLES = 16;
...
/** The core's ring grows from the small core's edge (15 su) to about its
 *  glow's reach (39 su): it used to grow 1.5 times from a 42 su disc. */
const CORE_RING = { scale: 2.6, peak: 0.35, steps: 0.4 } as const;

/** Entry/exit scale floor. */
const SCALE_HIDDEN = 0.55;

/**
 * How long each half of the wrap's swap takes, in steps: one wrap keyframe
 * (WRAP_SAMPLES), about 0.3s. The swap used to span the whole step, with the
 * words held back further (out over its first 30%, in over its last 30%),
 * which left one department's words missing or barely visible for most of
 * every step. The words ride inside their node, so its fade and scale are
 * theirs too. In turn rather than together, so one copy is always whole:
 * every department's number, label and role is on screen at full size and
 * strength at every moment, and in two places only for about 0.6s. The
 * exiting one is still gone before it reaches the crowded bottom of the arc
 * (slot 7 → 8 is 66 su, where a half-faded label brushed the one above it at
 * 1024 × 768).
 */
const WRAP_SWAP = 1 / 16;
...
function stateA(step: number): NodeState {
  const out = smooth((step - 7 - WRAP_SWAP) / WRAP_SWAP);
...
function stateB(step: number): NodeState {
  const inn = smooth((step - 7) / WRAP_SWAP);
  return {
...
/** The words ride inside their node, whose swap (WRAP_SWAP) they share, so
 *  their own opacity stays whole. */
const textOpacityA = () => 1;
const textOpacityB = () => 1;
...
export function buildNodeKeyframes(): string {
  ...
  const aText = aSteps.map((t): Frame => [
    pct(t),
    `transform:${textTransform(stateA(t).slot)};opacity:${textOpacityA()}`,
  ]);
  // The wrap step at the wrap's own sampling, so a thread swaps with its dot.
  const threadSteps = [
    ...range(0, 7, 1 / SAMPLES_PER_STEP),
    ...range(7, 8, 1 / WRAP_SAMPLES).slice(1),
  ];
  ...
  const bText = bSteps.map((t): Frame => [
    pct(t),
    `transform:${textTransform(stateB(t).slot)};opacity:${textOpacityB()}`,
  ]);
  const bThread = [0, ...range(7, 8, 1 / WRAP_SAMPLES)].map((t): Frame => {
    const s = stateB(t);
    return [
      pct(t),
      `transform:${threadTransform(pointAt(s.slot), THREAD_END_R)};opacity:${r3(s.opacity)}`,
